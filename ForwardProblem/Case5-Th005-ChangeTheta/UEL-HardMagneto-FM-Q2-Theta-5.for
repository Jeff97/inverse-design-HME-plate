...
!     cover MValue and Theta_M changes (modify path to select desired shape)
        IF (Cur_time .LE. 0.5) THEN
            Theta_M = 0.5*Pi*(1.0 - 2.0*GaussXc)
        ELSE
            Theta_M = 0.5*Pi*(1.0 - (1.0 + 2.0*Cur_time)*GaussXc)
        END IF
