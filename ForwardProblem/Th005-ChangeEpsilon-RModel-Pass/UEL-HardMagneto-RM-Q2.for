...
C       -- Deformation Gradient F --
        ...
        ! The RU decomposition of deformation gradient F
        call CALC_RU(F, R_Matrix, U_Matrix)

C       -- Current Magnetization MCur = J^(-1) * R . MRef --
        MCur(1) = (R_Matrix(1,1)*MRef(1)+R_Matrix(1,2)*MRef(2)
     &             +R_Matrix(1,3)*MRef(3))/DetF
        MCur(2) = (R_Matrix(2,1)*MRef(1)+R_Matrix(2,2)*MRef(2)
     &             +R_Matrix(2,3)*MRef(3))/DetF
        MCur(3) = (R_Matrix(3,1)*MRef(1)+R_Matrix(3,2)*MRef(2)
     &             +R_Matrix(3,3)*MRef(3))/DetF
...
! ======================================================================
!  SUBROUTINE: CALC_RU
!  Purpose: Calculate polar decomposition of deformation gradient F = R * U
!  Input:   F (3x3) - Deformation Gradient
!  Output:  R (3x3) - Rotation Tensor
!           U (3x3) - Right Stretch Tensor
!  Author:  Gemini (Based on User's Mathematica Algorithm)
!  Note:    Append this code block to the end of your ABAQUS UEL subroutine file.
! ======================================================================
      SUBROUTINE CALC_RU(F, R, U)
      IMPLICIT NONE
      ...
      ! --- Local Variables ---
      REAL*8 :: C(3,3)      ! Right Cauchy-Green Tensor
      REAL*8 :: U_INV(3,3)  ! Inverse of U
      REAL*8 :: Evals(3)    ! Eigenvalues of C (lambda^2)
      REAL*8 :: Evecs(3,3)  ! Eigenvectors of C
      REAL*8 :: lam(3)      ! Principal stretches (lambda)
      REAL*8 :: inv_lam     ! 1/lambda
      INTEGER :: i, j, k

      ! 1. Calculate Right Cauchy-Green Tensor C = F^T * F
      C = 0.0D0
      DO i = 1, 3
        DO j = 1, 3
          DO k = 1, 3
      ...
      ! 2. Spectral decomposition of C (Eigenvalue solution)
      !    Note: C is a real symmetric matrix
      CALL SPECTRAL_DECOMP_3X3(C, Evals, Evecs)

      ! 3. Calculate U and U_INV
      !    U = sum( sqrt(eval_i) * v_i * v_i^T )
      !    U_inv = sum( 1/sqrt(eval_i) * v_i * v_i^T )
      U = 0.0D0
      U_INV = 0.0D0

      DO k = 1, 3
          ! Ensure eigenvalues are positive (Theoretically C is PD, Evals > 0)
          ! Prevent small negative numbers due to numerical error
          IF (Evals(k) .LT. 1.0D-20) Evals(k) = 1.0D-20
          ...
          DO i = 1, 3
            DO j = 1, 3
              U(i,j) = U(i,j) + lam(k) * Evecs(i,k) * Evecs(j,k)
              U_INV(i,j) = U_INV(i,j) + inv_lam*Evecs(i,k)*Evecs(j,k)
            END DO
          END DO
      END DO

      ! 4. Calculate Rotation Tensor R = F * U_INV
      R = 0.0D0
      DO i = 1, 3
        DO j = 1, 3
          DO k = 1, 3
      ...
      RETURN
      END SUBROUTINE CALC_RU


! ======================================================================
!  SUBROUTINE: SPECTRAL_DECOMP_3X3
!  Purpose: Solve eigenvalues and eigenvectors of 3x3 real symmetric matrix using Jacobi method
!  Input:   A_IN (3x3) - Real Symmetric Matrix
!  Output:  W (3)      - Eigenvalues
!           V (3,3)    - Eigenvectors (Column vectors)
! ======================================================================
      SUBROUTINE SPECTRAL_DECOMP_3X3(A_IN, W, V)
      IMPLICIT NONE

      REAL*8, INTENT(IN)  :: A_IN(3,3)
      REAL*8, INTENT(OUT) :: W(3)
      REAL*8, INTENT(OUT) :: V(3,3)

      REAL*8 :: A(3,3)
      REAL*8 :: sm, t, c, s, tau, h, g, theta, t_val
      INTEGER :: p, q, i, iter
      ...
      DO iter = 1, MAX_ITER
          ...
          IF (sm .LT. 1.0D-12) RETURN ! Exit if converged

          DO p = 1, 2
            DO q = p + 1, 3
                g = 100.0D0 * DABS(A(p,q))
                
                IF (iter .GT. 4 .AND.
     &             (DABS(W(p)) + g .EQ. DABS(W(p))) .AND.
     &             (DABS(W(q)) + g .EQ. DABS(W(q)))) THEN
                    A(p,q) = 0.0D0
                ELSE IF (DABS(A(p,q)) .GT. 0.0D0) THEN
                    h = W(q) - W(p)
                    IF (DABS(h) + g .EQ. DABS(h)) THEN
                        t = A(p,q) / h
                    ELSE
                        theta = 0.5D0 * h / A(p,q)
                        t_val = 1.0D0 / (DABS(theta) 
     &                         + DSQRT(1.0D0 + theta**2))
          ...
          W(1) = W(1) ! No-op, W is updated incrementally above
      END DO

      END SUBROUTINE SPECTRAL_DECOMP_3X3