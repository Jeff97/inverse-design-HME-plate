...
!     swap (x,y) in Mathematica ArcTan[x,y] to align with FORTRAN ATan2(y,x)
      ...
      g0 = Finalg 
      Theta_M = Pi - ASIN((16.0*h**2 * g0) /
     &  (3.0*ABS(Epsilon - 2.0*h*(-1.0 + GaussXc)*Epsilon*g0))) -
     &  ATAN2((1.0 - 2.0*h*(-1.0 + GaussXc)*g0) *
     &  SIN((-2.0 + GaussXc)*GaussXc*g0 + Theta_b),
     &  (-1.0 + 2.0*h*(-1.0 + GaussXc)*g0) *
     &  COS((-2.0 + GaussXc)*GaussXc*g0 + Theta_b))
