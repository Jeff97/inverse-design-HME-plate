...
C     Parameters
      INTEGER, PARAMETER :: NELEMENT = 6000
      INTEGER, PARAMETER :: ELEMOFFSET = 100000
      INTEGER, PARAMETER :: nUvrm = 7
...
C     -- Field Variables --
      ...
      DOUBLE PRECISION :: Cur_time, TotalT, g0
...
!     cover Theta_M using GaussXc for X coordinate
!     swap (x,y) in Mathematica ArcTan[x,y] to align with FORTRAN ATan2(y,x)
      Theta_M = Pi - ASIN((512.0*h**2 * Pi * ((1.0/COSH((8.0/35.0)*
     &  (-42.0 + GaussXc)))**2 * TANH((8.0/35.0)*(-42.0 + GaussXc)) -
     &  2.0*(1.0/COSH((8.0/35.0)*(-28.0 + GaussXc)))**2 * 
     &  TANH((8.0/35.0)*(-28.0 + GaussXc)))) / (105.0 * 
     &  SQRT(Epsilon**2 * (4.0*h*Pi*(1.0/COSH((8.0/35.0)*(-42.0 + 
     &  GaussXc)))**2 - 8.0*h*Pi*(1.0/COSH((8.0/35.0)*(-28.0 + 
     &  GaussXc)))**2 + 35.0*(TANH(32.0/5.0) + TANH(48.0/5.0)))**2)))
     &  - ATAN2(Epsilon * SIN(Theta_b - (Pi*(TANH(48.0/5.0) + 
     &  TANH((8.0/35.0)*(-42.0 + GaussXc)))) / (2.0*(TANH(32.0/5.0) + 
     &  TANH(48.0/5.0))) + (Pi*(TANH(32.0/5.0) + TANH((8.0/35.0)*
     &  (-28.0 + GaussXc)))) / (TANH(32.0/5.0) + TANH(48.0/5.0))) * 
     &  (4.0*h*Pi*(1.0/COSH((8.0/35.0)*(-42.0 + GaussXc)))**2 - 
     &  8.0*h*Pi*(1.0/COSH((8.0/35.0)*(-28.0 + GaussXc)))**2 + 35.0*
     &  (TANH(32.0/5.0) + TANH(48.0/5.0))), 
     &  -Epsilon * COS(Theta_b - (Pi*(TANH(48.0/5.0) + 
     &  TANH((8.0/35.0)*(-42.0 + GaussXc)))) / (2.0*(TANH(32.0/5.0) + 
     &  TANH(48.0/5.0))) + (Pi*(TANH(32.0/5.0) + TANH((8.0/35.0)*
     &  (-28.0 + GaussXc)))) / (TANH(32.0/5.0) + TANH(48.0/5.0))) * 
     &  (4.0*h*Pi*(1.0/COSH((8.0/35.0)*(-42.0 + GaussXc)))**2 - 
...
C       -- TODO: SDVs for Visualization --
        ...
        GloUVar(JELEM,gp,4)  = Theta_M
        GloUVar(JELEM,gp,5)  = g0
      !   GloUVar(JELEM,gp,6)  = bExt(3)
      !   GloUVar(JELEM,gp,7)  = MRef(1)
      !   GloUVar(JELEM,gp,8)  = MRef(2)
      !   GloUVar(JELEM,gp,9)  = MRef(3)
        GloUVar(JELEM,gp,6) = Energy_Mech
        GloUVar(JELEM,gp,7) = Energy_Mag
