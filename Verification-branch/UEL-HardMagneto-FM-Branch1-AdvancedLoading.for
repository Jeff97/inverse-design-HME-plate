...
C     PROPS(5) - Finalg: Final bending angle (Rad)
...
!     swap (x,y) in Mathematica ArcTan[x,y] to align with FORTRAN ATan2(y,x)
      ! g0 = Finalg * Cur_time ! g0 is the bending angle that goes from 0 to prescribed
      
      Theta_M = ASIN((6.0*h**2 * Pi**2 * SIN((3.0*Pi*GaussXc)/2.0)) /
     &  SQRT((Epsilon + 1.5*h*Pi*Epsilon*COS((3.0*Pi*GaussXc)/2.0))
     &  **2)) - ATAN2(Epsilon * (1.0 + 1.5*h*Pi*COS((3.0*Pi*
     &  GaussXc)/2.0)) * SIN(Theta_b - SIN((3.0*Pi*GaussXc)/2.0)),
     &  -Epsilon * (1.0 + 1.5*h*Pi*COS((3.0*Pi*GaussXc)/2.0)) *
     &  COS(Theta_b - SIN((3.0*Pi*GaussXc)/2.0)))
     
C     External Fields (If coordinate is needed, can use GaussXc, GaussYc, GaussZc)

        Theta_b = (0.5 + 0.5* Cur_time) * PROPS(6)
